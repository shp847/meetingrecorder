...
- sustained specific Teams or Google Meet meeting-window evidence can auto-start a quiet call after a 20-second debounce only when audio is attributed to the meeting platform or the audio probe is unavailable; generic Teams and Google Meet shells require attributed app, window, process, or browser-tab audio even when endpoint-wide audio is active, preventing unrelated playback from creating short sessions
