...
The Updates section now treats local install facts and release-package facts as separate concepts. `Current Installation` is driven from managed install provenance so it reflects the machine’s actual installed-on time plus the installed package published-at and package size that the updater uses for same-version comparison, while the GitHub section keeps showing the remote package publish timestamp and installer asset size used for update comparison.
...
- persists install provenance under `%LOCALAPPDATA%\MeetingRecorder\install-provenance.json`, including the last installed-at timestamp plus the installed package published-at and asset-size identity used by in-app update comparison
