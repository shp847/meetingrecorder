...
16. Worker launches always run at reduced OS priority so background jobs yield to foreground work. In `Responsive` mode, new background queue work also pauses while a live recording is active, and primary publish can complete without waiting on optional speaker labeling when that mode is `Deferred`. In `Transcript only` speed profile, the queue can run up to two transcript-only workers, skips speaker labels and automatic summaries, and publishes transcripts first.
