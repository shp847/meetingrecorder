...
The Updates section now treats local install facts and release-package facts as separate concepts. `Current Installation` is driven from the running install root plus managed install provenance so it reflects the machine’s actual installed-on time and installed app footprint, while the GitHub section keeps showing the remote package publish timestamp and installer asset size used for update comparison.
