...
- Windows render-session audio probing classifies system sounds, inactive sessions, and the Meeting Recorder process before resolving process names, so endpoint tooling is less likely to see meeting detection as App.exe probing service-hosted Windows audio processes such as `svchost.exe`
