...
- sustained specific Teams or Google Meet meeting-window evidence can now auto-start a quiet call after a 20-second debounce even before render audio rises above the normal auto-start threshold; Teams still excludes generic/navigation and matching chat-playback surfaces, while Google Meet requires a stable Meet code or named `Meet - ...` title
...
- full cleanup refreshes can auto-apply safe fixes in the background; failed attempts and successfully dispatched transcript or speaker-label queue requests are suppressed per recommendation fingerprint until the recommendation changes, while manual actions remain available
