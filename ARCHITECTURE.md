...
- continuity protection for active auto-started Google Meet sessions when the Meet window is temporarily obscured by a weak Teams chat/navigation candidate and captured system audio is still active, using a bounded stop-timeout extension instead of resetting the positive-signal clock forever
...
That resume rule now includes one narrow Teams continuity case that had been splitting live calls: when the active session still has a specific Teams meeting title and Windows still attributes audio to that Teams session, or the audio probe times out while recent captured audio activity is still present, the app treats that as a real meeting continuation even if loopback level history is briefly quiet, so one-sided speaking stretches and short audio-probe gaps do not fragment into multiple sessions.
