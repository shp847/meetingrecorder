...
- keep speaker-labeling GPU acceleration explicit and opt-in: new installs default to CPU-only, legacy `Auto` configs migrate to CPU once, user-saved `Auto` choices are preserved, the worker packages the DirectML redistributable, and every DirectML failure falls back to CPU without blocking transcript publication
