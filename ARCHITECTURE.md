...
ModelProxy is treated as a local HTTP dependency, not an imported library or bundled service. Meeting Recorder uses Chat Completions for the first implementation because that is the ModelProxy-compatible contract, keeps the provider abstraction isolated so a hosted OpenAI path can evolve independently, sends only the transcript and minimal meeting metadata needed for a summary, discovers ModelProxy's local model catalog from `GET /v1/models`, uses `default_model` unless Settings has an explicit summary model override, disables web search for summary and validation requests, and handles `400`, `401`, `502`, `503`, timeout, and malformed-response cases as safe summary failures. Real transcript summaries through ModelProxy use at least a 240-second request timeout, while synthetic validation stays lightweight. The client reads ModelProxy routing headers (`X-ModelProxy-Request-Id`, requested/effective backend, web-search backend, app-server search support, and fallback reason) and exposes them as safe metadata. If a future ModelProxy request intentionally enables web search, the client tolerates CLI fallback and extends too-short web-search timeouts above ModelProxy's 45-second CLI-search bound. The streaming parser ignores SSE comment lines that start with `:` so keepalive/progress metadata cannot become assistant text, and terminal `event: error` frames are parsed as structured ModelProxy failures instead of endpoint connectivity failures. Forced app-server web-search `400` responses are treated as a capability message with a retry-without-web-search path.
