...
- Google Meet fallback no longer inspects Chromium tab UI directly; it now relies on explicit browser titles plus Windows render-session metadata from matching window/audio-session process IDs so Edge or Chrome are not probed through cross-process UI Automation during detection
...
- Windows render-session audio probing classifies system sounds, inactive sessions, and the Meeting Recorder process before deriving Teams/browser process labels from session metadata, so endpoint tooling is less likely to see meeting detection as App.exe probing service-hosted Windows audio processes such as `svchost.exe`
- Visible-window detection intentionally keeps process names blank and relies on supported window classes, titles, and render-session PID correlation instead of opening window owner processes, so service-hosted shell windows are not probed just to classify Teams or Meet surfaces
