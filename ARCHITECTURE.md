...
- full cleanup refreshes can auto-apply at most five safe fixes at a time, then refill in the background after the processing queue is idle and a 2-minute cooldown has elapsed; any previous automatic batch, including a partial batch, can unlock a later refill, and the Meetings tab does not need to stay selected; baseline row refreshes preserve the previous cleanup recommendation set until the replacement background scan completes, and the cleanup review banner reserves its row while hidden to avoid first-open layout jumps; normal Deferred primary-pass skips do not count as prior automatic attempts, while actual failures and successfully dispatched transcript or speaker-label queue requests are suppressed per recommendation fingerprint until the recommendation changes and manual actions remain available
