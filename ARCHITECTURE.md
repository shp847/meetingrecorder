...
- `MeetingRecorder.App`
  ...
  - local meeting detection for Teams, Google Meet, and calendar-matched Zoom Web calls, using visible window titles plus endpoint-level render audio activity without per-app Core Audio session enumeration
- Zoom Web detection requires positive Zoom evidence from the overlapping Outlook event, an exact event-subject/browser-title match, and active endpoint audio; it deliberately avoids browser UI Automation, URL scraping, and process/PID lookup
...
- continuity protection for active auto-started Google Meet sessions when the Meet window is temporarily obscured by a weak Teams chat/navigation candidate, including a three-minute silent grace for sessions pinned to a specific Meet code or named `Meet - ...` title, using a bounded stop-timeout extension instead of resetting the positive-signal clock forever
- managed cross-platform rollover requires platform-specific audio, official Teams meeting, or calendar-backed Zoom evidence; endpoint-only competing windows may sustain recent loopback continuity but cannot rename or split the active meeting
- named Google Meet identities discard Chromium tab-count/profile decoration before continuity comparison, while every `Sharing control bar` title variant is continuation-only
- sustained specific Teams or Google Meet meeting-window evidence can auto-start a quiet call after a 20-second debounce only when audio is attributed to the meeting platform or the audio probe is unavailable; generic Teams and Google Meet shells, including email-only Teams account windows, require attributed app, window, process, or browser-tab audio even when endpoint-wide audio is active, preventing unrelated playback from creating short sessions or cross-platform rollovers
- multiple distinct specific Teams candidates sharing only endpoint-wide audio are marked identity-ambiguous instead of being allowed to auto-start or roll over; recent captured loopback activity may keep the current Teams session alive without assigning that audio to a competing stale window title
- Teams detection now also treats newer in-call window titles that are just the attendee or meeting name as meeting surfaces when they come from a Teams host window, using the sustained debounce plus the existing playback/chat demotion path when endpoint render activity is unavailable
- Teams title cleanup removes a trailing organization plus email account decoration only when the last title segment is email-shaped, preserving ordinary pipe-delimited meeting names
...
- whether one explicit speaker-label repair must bypass deferred scheduling for its next attempt
...
- cleanup scheduling uses a persisted fingerprint ledger with pending, queued, processing, completed, failed, and manual-review states. Queue acceptance is not completion: worker outcome resolves queued repair work. Cleanup is low-priority and uses the existing worker cap; daytime scheduling alternates one cleanup item after normal work, while the configured overnight window allows bursts of up to five cleanup items before returning to normal work. Live recordings pause new background work, the scheduler refreshes independently of the selected Meetings tab, and manual actions remain available for failed or legacy-unprovable work. Baseline row refreshes preserve the previous cleanup recommendation set until the replacement background scan completes, and the cleanup review banner reserves its row while hidden to avoid first-open layout jumps.
