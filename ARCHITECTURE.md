...
The Updates section now treats local install facts and release-package facts as separate concepts. `Current Installation` reflects the machine’s actual installed-on time plus the on-disk install footprint, while installed package published-at and package size are shown from trusted managed install provenance when available, or temporarily mirrored from the latest successful `UpToDate` release check for display when an older install is missing provenance. The GitHub section keeps showing the remote package publish timestamp and installer asset size used for update comparison.
...
- persists install provenance under `%LOCALAPPDATA%\MeetingRecorder\install-provenance.json`, including the last installed-at timestamp plus any trusted installed package published-at and asset-size identity used by in-app update comparison, and both MSI post-install provisioning plus app startup now repair a missing provenance file with local install facts so older or partially migrated installs can recover gracefully
