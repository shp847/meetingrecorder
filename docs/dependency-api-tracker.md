...
| --- | --- | --- | --- | --- |
| Model Proxy local HTTP API | `GET /v1/models`, text-only non-streaming `POST /v1/responses`, bearer auth, `X-ModelProxy-Backend: codex`, `X-ModelProxy-Web-Search: false`, safe `X-ModelProxy-*` routing headers, and structured retryable `backend_busy` handling. | `%USERPROFILE%\OneDrive - Kearney\Documents\Coding Projects\Model Proxy\CONSUMER_INTEGRATION_HANDOFF.md`; `%USERPROFILE%\OneDrive - Kearney\Documents\Coding Projects\Model Proxy\CONFIG_REFERENCE.md`. | 2026-08-16 | Golden Consumer Contract v1: model records advertise `default` plus `reasoning_efforts`; Meeting Recorder follows the live default and sends `reasoning.effort` only when the user chooses an advertised concrete effort. Unsupported model or effort rejections refresh the catalog; only the default selection receives one retry. Catalog state is `live` or fallback/degraded, and `audio_capability=unsupported` is a hard boundary: Whisper transcription and diarization stay entirely local. |
...
- 2026-08-16: Meeting Recorder adopted ModelProxy Golden Consumer Contract v1:
  the live catalog selects the provider default, summary requests use the
  Codex/no-search text path, and ModelProxy audio remains unsupported.
