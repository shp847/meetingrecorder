...
Reliability fix: cleanup auto-apply no longer mistakes normal Deferred speaker-label skips for prior automatic attempts. Automatic cleanup uses a clean suppression cache, queues at most five fixes per idle batch, refills after a short cooldown even when the prior batch was partial, keeps draining when the Meetings tab is not selected, keeps the Meetings cleanup banner from shifting the list while background suggestions load, reuses one catalog snapshot per batch, and reports waiting versus queued or attempted work.
...
Reliability fix: the startup historical repair pass now reruns as `published-meeting-repair-v8` and can merge non-generic same-title Google Meet or Teams fragment chains when tight timing and at least one short fragment make the repair safe.
