...
- `Deferred` publishes audio and transcripts first and leaves `Add Speaker Labels` for later, `Throttled` runs speaker labeling automatically after transcription using the selected background budget, and `Inline` keeps speaker labeling in the primary pass for labeled output sooner.
