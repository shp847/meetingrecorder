...
- in-place updates now use the already-extracted temp bundle as the staging source when it is on the same drive as the managed install, then validate that staged source before moving any current install files; this avoids both a random sibling folder under `Documents` and a second unsigned apphost copy under `%TEMP%`
