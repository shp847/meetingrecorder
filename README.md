...
- `Deferred` publishes audio and transcripts first and leaves `Add Speaker Labels` for later. Legacy automatic speaker-labeling configs are migrated back to `Deferred` once to avoid endpoint prompts; `Throttled` and `Inline` remain explicit opt-ins for automatic labeling.
