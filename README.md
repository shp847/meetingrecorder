...
- Full Meetings cleanup refreshes auto-apply up to five current `Safe Fix` items at a time, then refill while `Meetings` remains open only after the processing queue is idle and a 15-minute cooldown has elapsed, while leaving review-first recommendations visible and manual bulk retry available
