...
- Flags likely errant meetings with cleanup recommendations such as archive, merge, split, rename, retry-transcript, add-speaker-label, repair-speaker-label, and missing-AI-summary actions. Summary generation stays `Review First` because it may use the configured local or hosted provider.
...
- On first launch after the updated build, the versioned `published-meeting-repair-v8` pass can collapse a heavily stop/start-fragmented same-call chain into one merged publish, including same-title chains that have longer useful anchors plus at least one short fragment, auto-merge a short-gap exact-title split when the preserved work manifests agree on the same specific meeting identity, republish repairable historical microphone sessions from durable `%LOCALAPPDATA%\MeetingRecorder\work` raw chunks when the previous v7 echo repair did not already run, and queue suspicious over-fragmented speaker labels for repair from the existing transcript text when speaker labeling is ready
...
- v2 in-app updates preserve the already-installed stable apphosts (`MeetingRecorder.App.exe`, `AppPlatform.Deployment.Cli.exe`, and `MeetingRecorder.ProcessingWorker.exe`) and update the mutable DLLs, assets, scripts, and manifests around them
- update packages are downloaded and staged under `%LOCALAPPDATA%\MeetingRecorder\updates` instead of the user Downloads folder, then fully validated before the app is asked to shut down
- legacy single-file installs that do not have the v2 `bundle-layout.json` marker require a one-time MSI or bootstrapper reset before in-app updates resume
...
- the default self-contained release now ships the WPF shell as a loose apphost plus `MeetingRecorder.App.dll`, `.deps.json`, and `.runtimeconfig.json`; the deployment CLI, worker, scripts, and product manifest remain external sidecars in the bundle
...
- Managed per-user mode
  ...
  - after a meeting publishes successfully, the app keeps the published recording as the retained audio copy, prunes no-longer-needed raw capture chunks, and deletes redundant bulky work-cache processing files when a matching published recording exists; startup also sweeps older published sessions that still retained those artifacts under `%LOCALAPPDATA%\MeetingRecorder\work`
...
- Google Meet auto-start still begins immediately when active browser-family audio is present; quiet-start debounce requires audio attributed to Google Meet or an unavailable audio probe, so a silent stale `Meet - ...` window cannot create repeated sessions
- Specific quiet Teams desktop meetings can auto-start after the same named meeting window remains visible for 20 seconds only when audio is attributed to Teams or the audio probe is unavailable; generic Teams and Google Meet shells also require attributed app or tab audio even when unrelated endpoint audio is active
- When several different specific Teams windows remain visible, endpoint-wide audio alone no longer chooses one arbitrarily: idle auto-start is suppressed, while an already active Teams recording continues only while captured loopback audio remains active
- Teams auto-detection now also recognizes newer in-call window titles that only show the attendee or meeting name without a visible `| Microsoft Teams` suffix, so quiet one-on-one and newer Teams call surfaces can qualify for the same sustained quiet-meeting auto-start; an email-only Teams account shell still requires audio attributed to Teams and cannot borrow unrelated endpoint playback
...
- Named Google Meet continuity also ignores changing Edge tab-count and profile decoration, and endpoint-only Teams windows cannot replace an active Meet unless Teams-specific audio or official meeting evidence confirms the handoff
- Account-decorated `Sharing control bar` windows remain continuation-only surfaces: they can sustain an active Teams call but cannot start a standalone recording
...
- Separate meeting detail window showing readable transcript paragraphs from JSON sidecars with Markdown fallback, while preserving raw JSON segments for structured processing, plus meeting facts, attendees, recommendation badges, transcript model metadata, speaker-label state, the persisted detected audio source summary, and collapsed capture diagnostics
...
- Safe cleanup work uses a persisted scheduler ledger. It keeps the current worker limit, pauses during live recording, alternates one low-priority cleanup item after normal work during the day, and clears up to five-item bursts in the configured overnight window. The scheduler runs even when Meetings is not selected, and dispatches only after its baseline refresh has released the UI busy state. The banner separates eligible, queued, processing, disabled speaker-label, provider-blocked summary, disabled cleanup, and manual-review work, then shows the latest scheduler dispatch or blocker reason. A configured local speaker-labeling bundle enables deferred speaker labels during the one-time migration; you can opt out in Settings.
...
- If the processing worker crashes inside optional speaker labeling, the queue now stamps that manifest with an internal skip-label override, retries it once without diarization, switches future speaker labeling back to `Deferred`, persists a durable per-session transcript snapshot before speaker labeling begins, repairs older stale post-transcription sessions on startup, and prioritizes those already-transcribed repaired sessions ahead of untouched queue items so audio and transcript publishing can still complete instead of leaving meetings stranded in the backlog
- If source-audio preparation cannot build a usable WAV, the worker now marks the manifest `Failed` instead of leaving it `Queued`; when microphone merging fails but loopback audio is still readable, the merge falls back to loopback-only audio so transcript publishing can continue
- `Settings > Advanced` now exposes `Background processing mode` (`Light`, `Balanced`, `Fast`, `Maximum`), `Speaker labeling mode` (`Deferred`, `Throttled`, `Inline`), and `Processing speed profile` (`Normal`, `Transcript only`, `Overnight transcript only`) so you can trade backlog drain speed against overall machine responsiveness. Each background-mode dropdown label includes the current PC's transcription and speaker-labeling thread budget.
- Background processing workers now launch at reduced OS priority in every processing mode so transcript jobs yield to foreground work. The default performance profile is `Light` plus `Deferred`, which also pauses new background queue work while a live recording is active, caps worker thread usage, and lets audio/transcript publish finish before optional speaker labeling. `Maximum` skips the live-recording pause and caps local work at up to 12 transcription threads and 6 speaker-labeling threads while still keeping worker processes below normal priority.
- `Transcript only` skips automatic speaker labeling and summaries, publishes audio/transcript artifacts first, and allows up to two transcript-only workers at once; `Overnight transcript only` applies that mode during the configured local overnight window. Optional speaker labeling runs in a bounded helper process: after 45 minutes only the native label pass is stopped, the existing transcript still publishes, and the meeting reports manual review instead of leaving the queue stuck. Repairable PCM WAV headers are normalized only in a temporary copy; source recordings are never rewritten.
- Experimental transcription and diarization replacement backends are available in `Settings > Advanced` as user-provided local CLI providers. The worker probes the configured executable with `--probe`, persists the result, passes prepared WAV/transcript files through `{audioPath}` and `{transcriptPath}` placeholders, expects current `TranscriptionResult` or `DiarizationResult` JSON on stdout, and falls back to built-in Whisper.NET or Sherpa providers until the selected CLI has a successful matching probe.
- `Deferred` publishes audio and transcripts first and leaves `Add Speaker Labels` for later. Legacy automatic speaker-labeling configs are migrated back to `Deferred` once to avoid endpoint prompts; after that migration, saving Settings preserves explicit `Throttled` and `Inline` opt-ins.
- The shell now exposes queue-state visibility for that throttled backlog: the header chip stays visible whenever backlog exists, and the Meetings processing strip shows pause reasons such as `Paused by live recording`, the active item, current stage, elapsed processing time, and approximate per-item plus overall queue ETAs. Native diarization writes content-free heartbeats with its selected route, candidate pass, input duration, and memory footprint; until it has a reliable completed-pass estimate, the UI shows `ETA learning` rather than a false countdown. When only persisted queued/processing rows are available, the shell still surfaces the backlog and labels the ETA as unavailable until the live worker snapshot reconnects.
- When one meeting is marked `ASAP`, that same header chip and Meetings strip now call out the rushed title, whether it is bypassing the live-recording pause rule, and whether another interrupted item was safely requeued behind it. The rushed request is persisted in app config so it survives restart until the meeting finishes or you clear it. `Rush Backlog...` applies the transcript-first skip-label override to queued backlog items, and if the active worker is already in speaker labeling it interrupts and requeues that item so the persisted transcript snapshot can publish without labels; active transcription is left running.
...
- Successful publishes now also normalize the session manifest onto the retained published audio, prune raw capture chunks from the work-session `raw` folder, and delete redundant bulky `processing` files when a matching published recording exists. Startup reclaims the same work-cache artifacts from older already-published sessions so `%LOCALAPPDATA%\\MeetingRecorder\\work` does not grow without bound.
...
Automatic cleanup recommendations and safe fixes remain archive-first. The app moves suspicious or superseded meeting artifacts into the Meetings archive so they can be recovered later if needed. Current builds use a single `Documents\Meetings\Archive` root for archive-style actions and one-time repair flows, and older parallel legacy roots such as `ArchivedRepairs` or `ArchivedFalseStarts` can be consolidated under that same `Archive` stem. Archived files are also marked unpinned on Windows so OneDrive-backed meeting folders can reclaim local disk space while keeping the backup artifacts recoverable from the cloud. Cleanup state is persisted by recommendation fingerprint as pending, queued, processing, completed, failed, or manual review. The background work plan explicitly selects queued recordings, deferred labels, missing AI summaries, and safe cleanup; selected work runs when idle and receives bursts of up to five during the configured overnight window without exceeding the selected resource-usage limit. Live recording pauses new background work in Light mode, the scheduler runs without the Meetings tab selected, and the banner distinguishes waiting, queued, processing, and manual-review work. Missing-summary work requires an enabled, validated provider and records provider failures for manual review rather than silently retrying. Failed work and legacy queue records without durable proof remain manual review; `Apply Safe Fixes` remains the explicit retry path. Auto-generated repair backup folders such as `published-meeting-repair-v*` and timestamped `*-echo-repair-*` archives are pruned after 14 days; manual meeting archives are not included in that automatic retention cleanup. Separately, successful publish and startup maintenance can permanently delete redundant bulky local work-cache files only after the matching published recording exists, without deleting published recordings or transcripts.

Split-meeting cleanup keeps the normal two-minute merge limit. It can bridge up to five minutes only for Teams fragments whose preserved manifests contain the exact same specific meeting title and window identity, with at least one fragment no longer than three minutes; longer recurring calls remain separate.
...
- Speaker diarization automatically estimates anonymous speakers from voice embeddings. It starts with the default clustering threshold, retries lower thresholds when voices collapse below two supported speakers, retries progressively higher thresholds when clustering over-segments, and runs a merge rescue for acoustically similar over-split clusters before deciding an over-segmented catalog is unsafe. It still skips bad labels instead of publishing unusable speaker catalogs, and it drops tiny fragment-only speaker IDs from the published catalog. Already-published meetings with a suspicious speaker explosion can be queued for `Repair Speaker Labels`; the repair seeds the worker with the existing transcript text stripped of old labels so it re-runs diarization without full retranscription.
- `scripts\Analyze-Diarization.ps1` provides a local calibration loop for known examples: point it at a transcript JSON sidecar, pass `-ExpectedSpeakerCount 2`, and it reports safe label counts/run counts without printing transcript text or modifying manifests. The expected count is a test-pass condition for evaluation, not an input to normal diarization.
- `scripts\Test-DiarizationFixture.ps1` provides a faster no-deploy replay for tuning the cluster-merge stage against known examples. Pass the published transcript JSON, the matching work manifest when available, and an expected count used only as an assertion; the script runs one focused test and reports before/after speaker counts without launching the app, worker, installer, or mutating meeting files.
- `scripts\Test-DiarizationFullAudioFixture.ps1` provides the slower no-deploy end-to-end fixture probe for examples that do not already have speaker turns. It copies the persisted transcript snapshot into a temporary session, references the full source audio read-only, clears only the temporary skip-speaker-labeling override, writes recordings/transcripts under a `probe-output` folder, can hash protected published artifacts before and after the run, and can assert expected speaker names when `-EnableSpeakerNameMatching` is explicitly supplied.
- Optional local voice-profile memory can learn names from user-confirmed speaker-label corrections, store embeddings under `%LOCALAPPDATA%\MeetingRecorder\speaker-profiles`, auto-apply only mature high-confidence future matches, show lower-confidence suggestions with reasons, and let you reject or undo bad meeting-specific matches without deleting or globally banning a voice profile. Voice-profile embeddings are not exported in transcript JSON.
- GPU acceleration for speaker labeling is modeled as an explicit DirectML opt-in. Meeting Recorder owns and ships the native dependency: release engineering builds the pinned DirectML-enabled Sherpa runtime with `scripts/Build-SherpaDirectMlRuntime.ps1`, stores it under `assets\native\sherpa-onnx-directml\win-x64`, and `scripts/Publish-Portable.ps1` bundles it into the installer payload. The app still falls back to CPU with a clear status if a managed device blocks DirectML.
...
- `Settings > Setup` now also exposes `When to run speaker labeling`, so an existing install can switch directly between `Deferred`, `Throttled`, and `Inline` without leaving Setup. Choosing or importing a speaker-labeling bundle keeps the current run mode instead of auto-promoting `Deferred` to automatic labeling.
...
- Speaker-name learning is local-only and user-controlled from `Settings > General`, including disable, selected-profile delete, and delete-all controls. The meeting detail speaker-name editor shows voice-profile provenance, can accept/reject suggestions, can `Refresh Suggestions` against improved local profiles without retranscribing audio or re-running diarization, and can `Undo Name Recognition` to clear profile-applied names for that meeting while preserving explicit user edits. Use `Repair Speaker Labels` only for suspicious diarization output; it is the heavier path that re-runs speaker labeling from the existing transcript snapshot. Voice-profile embeddings can be sensitive voice-derived data; review local policy before teaching names for other people.
...
If startup finishes resolving or switching to a healthy Whisper model a moment after the window first appears, `Home` now refreshes the manual `START` state immediately instead of waiting for a relaunch or another navigation cycle.
...
The live capture-status well under `Detected audio source` keeps the active loopback endpoint visible in plain language, shows whether the app is in `Loopback live`, `Loopback + mic live`, `Swapping loopback`, or `Fallback capture active` mode, and lists the most recent capture events so users can tell when Windows moved the meeting onto a different playback device. Microphone capture swaps are only shown for real input changes; Windows role-only flips for the same microphone stay quiet to avoid churn.
...
- Retrieve provider model choices from `GET /v1/models`. ModelProxy advertises its default and supported reasoning efforts; Settings preserves a configured unknown choice as an explicit Custom model.
- Summary reasoning defaults to Medium. Provider default omits reasoning from the Responses payload, while unavailable effort choices remain disabled instead of being ignored.
- Send summary and validation calls through the text-only, non-streaming `POST /v1/responses` subset. Requests use `X-ModelProxy-Backend: codex` and `X-ModelProxy-Web-Search: false`, so private transcript enrichment stays limited to supplied text while ModelProxy manages its own safe Codex fallback.
- Read ModelProxy's safe routing headers on local responses, including request id, requested/effective backend, web-search backend, and fallback reason. These safe headers are preserved in summary provider metadata when present.
- Treat structured `backend_busy` responses as retryable ModelProxy saturation, not as an unreachable LLM endpoint. If retries are exhausted, show a temporary saturation failure. Structured model or reasoning rejections refresh the catalog; only the live-default selection is retried once.
- Keep Whisper transcription and diarization entirely local. ModelProxy advertises `audio_capability=unsupported`; Meeting Recorder never sends audio to it.
- Allow local ModelProxy transcript summary requests at least 120 seconds to finish; Settings validation remains a lightweight synthetic request.
...
- caller model: the live ModelProxy catalog default unless `-Model` is supplied
- summary requests must send `X-ModelProxy-Backend: codex` and `X-ModelProxy-Web-Search: false`

Use `MODELPROXY_MEETING_RECORDER_API_KEY` only if your local ModelProxy operator config uses a non-default local key. The script discovers the live default, sends synthetic no-search `codex` requests, treats retryable `backend_busy` as temporary saturation, and prints only success state plus safe ModelProxy routing headers. It does not print prompt text, response text, API keys, bearer headers, or meeting/private context. OpenAI API keys, when configured later through Settings, must be stored outside plaintext app config and never echoed into logs, status text, or transcript artifacts.

Settings includes summary provider configuration, discovery-backed model selection with an explicit Custom model option, a Medium-default reasoning effort, DPAPI-protected OpenAI key storage under the app data root, and synthetic validation that asks providers to return `summary-provider-ok` without sending transcript, attendee, client, or meeting content. Validation status is shown inline for the provider being tested. Automatic summary generation runs in the processing worker, and the meeting detail window can generate or retry summaries from existing transcript JSON artifacts. Credential clearing remains in Settings for hosted OpenAI.
