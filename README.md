...
- Managed per-user mode
  ...
  - after a meeting publishes successfully, the app keeps the published recording as the retained audio copy, prunes no-longer-needed raw capture chunks, and deletes redundant work-cache processing WAVs when a matching published recording exists; startup also sweeps older published sessions that still retained those artifacts under `%LOCALAPPDATA%\MeetingRecorder\work`
...
- Windows audio-session probing now classifies system sounds, inactive sessions, and the app's own audio before deriving Teams/browser process labels from session metadata, reducing endpoint-protection prompts from service-hosted Windows audio sessions such as `svchost.exe`
- Window-based meeting detection now avoids resolving visible-window process names; it uses supported window classes, titles, and render-session process IDs instead, so service-hosted browser or Teams shell windows cannot trigger protected `svchost.exe` process-memory prompts
...
- Successful publishes now also normalize the session manifest onto the retained published audio, prune raw capture chunks from the work-session `raw` folder, and delete redundant `processing` WAVs when a matching published recording exists. Startup reclaims the same work-cache artifacts from older already-published sessions so `%LOCALAPPDATA%\\MeetingRecorder\\work` does not grow without bound.
