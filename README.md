...
- in-place updates now stage the copied bundle under the installer temp area when it is on the same drive as the managed install, then validate that staged copy before moving any current install files; this avoids dropping unsigned apphosts into a random sibling folder under `Documents`
