...
public sealed class MeetingRecorderModelCatalogServiceTests
{
    ...
    public void LoadBundledCatalog_Falls_Back_To_The_Default_Curated_Model_Catalog()
    {
        ...
        Assert.Equal("ggml-base.en-q8_0.bin", catalog.Transcription.Standard.FileName);
        Assert.Equal("Standard", catalog.Transcription.Standard.Label);
        ...
        Assert.Equal("meeting-recorder-diarization-bundle-standard-win-x64.zip", catalog.SpeakerLabeling.Standard.FileName);
        Assert.Equal("Standard", catalog.SpeakerLabeling.Standard.Label);
    ...
    public void ResolveManagedPath_Uses_The_Curated_Runtime_Locations()
    {
        ...
        var transcriptionPath = service.ResolveManagedPath(modelCacheDir, catalog.Transcription.Standard);
        var speakerLabelingPath = service.ResolveManagedPath(modelCacheDir, catalog.SpeakerLabeling.Standard);
