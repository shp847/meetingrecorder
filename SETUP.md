...
For Google Meet, the detector still tries to prove audio belongs to the Meet tab first, and an explicit active `Meet - ...` browser window can auto-start from active browser-family audio even when browser session metadata is too weak to name the exact tab. Quiet-start debounce requires audio attributed to Google Meet or an unavailable audio probe, so a silent stale Meet window cannot create repeated fragments.
...
- failed automatic safe-fix attempts and successfully dispatched retry-transcript or speaker-label queue requests are suppressed by recommendation fingerprint until the recommendation changes, but manual `Apply Safe Fixes` still tries them immediately
