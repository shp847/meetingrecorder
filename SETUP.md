...
- automatic cleanup queues at most five safe fixes at a time, waits for the processing queue to become idle, and enforces a 2-minute cooldown before refilling in the background; partial batches also unlock later refills when new eligible fixes remain, the Meetings tab no longer needs to stay selected, and `Responsive` processing still uses one below-normal worker and pauses new background work while recording
