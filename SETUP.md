...
For Google Meet, the detector still tries to prove audio belongs to the Meet tab first, and an explicit active `Meet - ...` browser window can auto-start from active browser-family audio even when browser session metadata is too weak to name the exact tab. When that same explicit Meet evidence is silent, the app waits for 20 seconds of stable joined-Meet identity, accepting either a Meet code or Google's named meeting title, so brief browser-title flashes do not create tiny fragments.
...
The detector treats a plain Teams content window cautiously when a matching `Chat | ... | Microsoft Teams` surface is present at the same time. A separate specific Teams meeting window can still auto-start after it remains stable for 20 seconds even while render audio is quiet; generic shell and navigation titles stay excluded.
