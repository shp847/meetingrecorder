...
- automatic cleanup queues at most five safe fixes at a time and, while `Meetings` remains open, waits for the processing queue to become idle and enforces a 2-minute cooldown before refilling; partial batches also unlock later refills when new eligible fixes remain, and `Responsive` processing still uses one below-normal worker and pauses new background work while recording
