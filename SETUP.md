...
For Google Meet, the detector still tries to prove audio belongs to the Meet tab first, and an explicit active `Meet - ...` browser window can auto-start from active browser-family audio even when browser session metadata is too weak to name the exact tab. Generic browser shells such as `Google Meet and 1 more page` require audio attributed to the Meet tab instead of borrowing unrelated endpoint playback. Quiet-start debounce requires audio attributed to Google Meet or an unavailable audio probe, so a silent stale Meet window cannot create repeated fragments.
For Google Meet recordings that are already active and tied to a specific Meet code or named `Meet - ...` title, a brief switch to a Teams chat, calendar, or search surface no longer immediately ages the Meet into auto-stop while captured system audio is active or while the current audio probe is silent; the app uses a bounded grace period of up to three minutes so short focus changes do not fragment the same call.

Named Meet titles remain one meeting when Edge changes only its `and N more pages` or Work/Personal profile decoration. A visible Teams window using only endpoint-wide audio cannot replace that active Meet; Teams-specific audio or official meeting evidence is required. `Sharing control bar` variants can continue an existing Teams recording but never start one by themselves.
...
- speaker-labeling DirectML preference, bundled runtime support, and CPU fallback policy
- update-check toggle
- auto-install toggle
- update feed URL
...
The detector treats a plain Teams content window cautiously when a matching `Chat | ... | Microsoft Teams` surface is present at the same time. A separate specific Teams meeting window can auto-start after it remains stable for 20 seconds only when audio is attributed to Teams or the audio probe is unavailable; generic shells such as `Microsoft Teams | Pinned window` and email-only Teams account windows require attributed Teams audio and cannot borrow unrelated endpoint playback.
...
3. Zoom Web can auto-start when the overlapping Outlook event contains Zoom join evidence, its subject exactly matches the visible Chromium window title, and endpoint audio is active. Native, unscheduled, or title-mismatched Zoom calls, Webex, and other conferencing apps still require manual start.
...
- automatic cleanup queues at most five safe fixes at a time, waits for the processing queue to become idle, and enforces a 2-minute cooldown before refilling in the background; partial batches also unlock later refills when new eligible fixes remain, the Meetings tab no longer needs to stay selected, the cleanup banner keeps its space reserved while background suggestions load, and `Responsive` processing still uses one below-normal worker and pauses new background work while recording
...
- on first launch after the updated build, the versioned `published-meeting-repair-v8` pass can now merge longer same-title split chains from repeated auto-stop / auto-start churn when the chain includes at least one short fragment, auto-merge a short-gap exact-title split when the matching work manifests still point at the same specific meeting window/title evidence, republish repairable historical microphone sessions from `%LOCALAPPDATA%\MeetingRecorder\work` only when the previous v7 echo repair did not already run, and queue suspicious over-fragmented speaker labels for repair when speaker labeling is ready
...
- if a previous app instance crashed while a recording manifest was still open, the next startup now stamps an end time from the newest preserved audio chunk, requeues that session, and lets the normal publish pipeline recover it
- the hidden in-app activity log now keeps a bounded rolling UI view and disables TextBox undo history, so repeated detection status changes during long-running sessions cannot grow WPF UI-thread state without bound; the disk log remains complete
...
- Assisted auto-detection supports Teams desktop, Google Meet, and calendar-matched Zoom Web calls. Native, unscheduled, or title-mismatched Zoom calls, Webex, and other conferencing apps should be started manually.
