...
- silent in-app updates may repair an existing pinned taskbar shortcut because that does not create a new shell surface; it only rewrites an already-pinned `Meeting Recorder.lnk` to the installed app executable and installed `MeetingRecorder.ico` when the old pin references the same install root or a missing stale target, and it must not repoint a healthy pin to an unrelated or temporary test install
...
16. Confirm a later CLI/in-app update preserves the current transcription profile, speaker-labeling profile, and speaker-labeling run mode while restoring missing bundled Standard assets if needed
